...
export async function POST(req: Request) {
  try {
    ...
    // Move heavy lifting into background using Next.js `after`
    after(async () => {
      try {
        ...
        // 2. Execute DB Status Update and File Download Concurrently
        const statusUpdatePromise = supabaseAdmin
          .from('contracts')
          .update({ status: 'analyzing' })
          .eq('id', contractId);

        const fileDownloadPromise = supabaseAdmin.storage
          .from('contracts')
          .download(contract.file_path);

        const [_, downloadResult] = await Promise.all([statusUpdatePromise, fileDownloadPromise]);
    ...
    // Send instant response so the frontend user continues tracking immediately
